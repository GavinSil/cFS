...
│  TIME: 1HzBoundary + ToneSignal                         │
│        ToneSemConsume + Local1HzSemConsume               │
│  SCH:  MinorFrame + SendTrigger + DispatchComplete       │
├─────────────────────────────────────────────────────────┤
│                OSAL POSIX Hook 层                        │
...
2. **事件描述统一**：18 种事件共用同一个 `ESA_Stepping_ShimEvent_t` 结构体，4 个字段即可描述所有事件。
...
typedef struct
{
    ESA_Stepping_EventKind_t event_kind;     /* 事件类型（18 种之一） */
...
**设计取舍**：4 个字段足以覆盖所有 18 种事件。`entity_id` 的语义随事件类型变化 — 对于 QueueReceive 它是队列 ID，对于 BinSemTake 它是信号量 ID，对于 CoreService 它是服务枚举值。这种"多态字段"避免了使用 union 或继承，保持了结构体的简单性。
...
| 事件 | 索引 | 含义 |
|------|------|------|
| `TASK_DELAY_ACK` | 0 | TaskDelay 阻塞操作进入前确认 |
| `TASK_DELAY_COMPLETE` | 1 | TaskDelay 阻塞操作完成 |
| `QUEUE_RECEIVE_ACK` | 2 | OS_QueueGet() 进入前确认 |
| `QUEUE_RECEIVE_COMPLETE` | 3 | OS_QueueGet() 返回后完成 |
| `BINSEM_TAKE_ACK` | 4 | OS_BinSemTake() 进入前确认 |
| `BINSEM_TAKE_COMPLETE` | 5 | OS_BinSemTake() 返回后完成 |

#### SCH 调度器事件（4 种）

| 事件 | 索引 | 含义 |
|------|------|------|
| `SCH_MINOR_FRAME` | 6 | SCH 开始次要帧处理（**唯一时间推进点**） |
| `SCH_SEND_TRIGGER` | 7 | SCH 向目标应用发送调度消息 |
| `SCH_DISPATCH_COMPLETE` | 8 | SCH 一轮调度消息发送完毕 |
| `SCH_NOOPS_DISPATCH_COMPLETE` | 9 | SCH 调度轮次中无消息发送 |

#### cFE 核心服务事件（2 种）

| 事件 | 索引 | 含义 |
|------|------|------|
| `CORE_SERVICE_CMD_PIPE_RECEIVE` | 10 | cFE 核心服务收到命令管道消息 |
| `CORE_SERVICE_CMD_PIPE_COMPLETE` | 11 | cFE 核心服务处理完命令管道消息 |
...
| 事件 | 索引 | 含义 |
|------|------|------|
| `TIME_1HZ_BOUNDARY` | 12 | TIME 检测到 1Hz 边界 |
| `TIME_TONE_SIGNAL` | 13 | TIME 收到 Tone 信号 |
| `TIME_TONE_SEM_CONSUME` | 14 | TIME Tone 子任务获取信号量 |
| `TIME_LOCAL_1HZ_SEM_CONSUME` | 15 | TIME Local1Hz 子任务获取信号量 |

#### 系统事件（2 种）

| 事件 | 索引 | 含义 |
|------|------|------|
| `TASK_DELAY_RETURN` | 16 | TaskDelay 到期返回（用于 TaskDelay 接管机制） |
| `TASK_DELAY_ELIGIBLE_QUERY` | 17 | 查询任务是否参与 TaskDelay 接管 |

### 3.4 单入口分发

整个 Shim 层在 PSP 侧的实现（`esa_stepping.c`）中表现为一个 18 路 switch 语句：
...
TaskDelay 事件包含 ACK（确认）和 COMPLETE（完成）两个阶段，用于精确跟踪任务何时进入和退出等待状态。
...
**NOOPS 分支**：如果 SCH 在一个次要帧中没有发送任何调度消息，报告 `SCH_NOOPS_DISPATCH_COMPLETE`（索引 9），Core 当前不对此事件做特殊处理，但保留了扩展接口。
...
| 文件 | 符号 | 类别 | 说明 |
|------|------|------|------|
| `esa/public_inc/esa_stepping_shim.h` | `ESA_Stepping_EventKind_t` | 🆕 枚举 | 18 种 Shim 事件类型，涵盖 TASK_DELAY（含 ACK/COMPLETE）、QUEUE_RECEIVE（含 ACK/COMPLETE）、BINSEM_TAKE（ACK/COMPLETE）、CORE_SERVICE（ACK/COMPLETE）、TIME 两事件（1HZ_BOUNDARY / TONE_SIGNAL）、TIME 信号量消费两事件、SCH 四事件（MINOR_FRAME / SEND_TRIGGER / DISPATCH_COMPLETE / NOOPS_DISPATCH_COMPLETE）、SYSTEM_READY 等 |
...
| | `ESA_Stepping_Shim_ReportEvent()` | 🆕 | 18-case switch：将 `ShimEvent` 按事件类型路由到对应的 Core API 调用 |
...
| 文件 | 函数 | 类别 | 说明 |
|------|------|------|------|
| `cfe/modules/time/fsw/src/cfe_time_stepping.h` | `CFE_TIME_Stepping_Hook_1HzBoundary()` | 🆕 | 声明：1Hz 边界 hook |
| | `CFE_TIME_Stepping_Hook_ToneSignal()` | 🆕 | 声明：Tone 信号 hook |
| | 无操作宏（`CFE_SIM_STEPPING` 未定义时） | 🆕 | 两个同名宏展开为空语句，实现零开销禁用 |
| `cfe/modules/time/fsw/src/cfe_time_stepping.c` | `CFE_TIME_Stepping_Hook_1HzBoundary()` | 🆕 | 构造 ShimEvent（`TIME_1HZ_BOUNDARY`），调用 Shim |
...
| 文件 | 函数/位置 | 类别 | 变更描述 |
|------|-----------|------|----------|
| `cfe/modules/time/fsw/src/cfe_time_task.c` | 1Hz 边界处理 | ✏️ | 在 1Hz 处理路径调用 `CFE_TIME_Stepping_Hook_1HzBoundary()` |
...
调度器在次帧回调、分发完成两个时序关键点注入步进事件，使步进引擎能感知调度周期的起止边界。

#### 10.5.1 新增文件

| 文件 | 函数 | 类别 | 说明 |
|------|------|------|------|
| `apps/sch/fsw/src/sch_stepping.h` | `SCH_Stepping_Hook_MinorFrame()` | 🆕 | 声明 + 无操作内联回退（`CFE_SIM_STEPPING` 未定义时零开销） |
| `apps/sch/fsw/src/sch_stepping.c` | `SCH_Stepping_Hook_MinorFrame()` | 🆕 | 构造 ShimEvent（`SCH_MINOR_FRAME`），调用 Shim |
...
esa/ (🆕 整个模块)
  ├─ public_inc/esa_stepping_shim.h ······ Shim ABI（18 种事件 + 统一载荷）
...
cfe/
  ├─ modules/time/fsw/src/cfe_time_stepping.h · 🆕 2 hook 声明 + no-op 宏
  ├─ modules/time/fsw/src/cfe_time_stepping.c · 🆕 2 hook 实现
  ...
  ├─ modules/time/fsw/src/cfe_time_task.c ···· ✏️ +2 stepping hook 调用
...
apps/sch/
  ├─ fsw/src/sch_stepping.h ·················· 🆕 1 hook 声明 + inline no-op
  ├─ fsw/src/sch_stepping.c ·················· 🆕 1 hook 实现
  ├─ fsw/src/sch_custom.c ···················· ✏️ +MinorFrame hook
